...
def summarize(G_hats, correct_flags, G_true):
...
def sweep_G(G_true_values, n_seeds=N_SEEDS):
    ...
    for G_true in G_true_values:
        ...
        for seed in range(n_seeds):
            ...
            prob, G_hat, _ = detect_major_G_closedform(
                mfg, X, true_idx,
                n_em_iters=N_EM_ITERS, n_inner_E_steps=N_INNER_E_STEPS,
                lam_entropy=LAM_BASE, verbose=False,
            )
            G_hats.append(G_hat.item())
            correct_flags.append(int(prob.argmax()) == true_idx)
        row = {"G_true": G_true, "n_seeds": n_seeds, **summarize(G_hats, correct_flags, G_true)}
        rows.append(row)
    print()
    return rows
...
def sweep_N(N_values, n_seeds=N_SEEDS):
    ...
    for N in N_values:
        ...
        for seed in range(n_seeds):
            ...
            G_hats.append(G_hat.item())
            correct_flags.append(int(prob.argmax()) == true_idx)
        row = {"N": N, "n_seeds": n_seeds, **summarize(G_hats, correct_flags, G_TRUE_BASE)}
        rows.append(row)
    print()
    return rows
...
def sweep_lambda(lam_values, n_seeds=N_SEEDS):
    mfg = make_mfg(G_TRUE_BASE)
    per_lam = {lam: {"G_hats": [], "correct_flags": []} for lam in lam_values}
    ...
    for lam in lam_values:
        row = {"lam_entropy": lam, "n_seeds": n_seeds,
               **summarize(per_lam[lam]["G_hats"], per_lam[lam]["correct_flags"], G_TRUE_BASE)}
        rows.append(row)
    print()
    return rows
